...
from detector.utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy
from detector.utils.torch_utils import is_parallel
...
class ComputeLossV26:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLossV26, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            top_k, _ = torch.topk(pair_wise_iou, min(10, pair_wise_iou.shape[1]), dim=1)
            dynamic_ks = torch.clamp(top_k.sum(1).int(), min=1)
            gt_wh = this_target[:, 4:6] * imgs[batch_idx].shape[1]
            small_mask =(gt_wh[:,0] <self.stride[0]) | (gt_wh[:,1] <self.stride[0])
            if small_mask.any():
                dynamic_ks[small_mask] =torch.clamp(dynamic_ks[small_mask] *2 ,max=pair_wise_iou.shape[1])
            num_gt = this_target.shape[0]
            gt_cls_per_image = (
                F.one_hot(this_target[:, 1].to(torch.int64), self.nc)
                .float()
