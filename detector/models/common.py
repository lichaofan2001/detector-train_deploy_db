...
from detector.utils.general import non_max_suppression, make_divisible, scale_coords, increment_path, xyxy2xywh
from detector.utils.plots import color_list, plot_one_box
...
# 形状大小切换，将形状大小变为原来的1/4，即相邻的四个像素合并到通道维度    
...
# 将数据沿着指定维度进行切分，ADD之后再连接，维度相比于直接concat，降低为原来的1/2
class Chuncat(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = []
        for xi in x:
            xi1, xi2 = xi.chunk(2, self.d) # 沿着某个维度进行切分
            x1.append(xi1)
            x2.append(xi2)
...
# 改进的CBS模块，先进行CBS操作，但是通道数量不变，再利用1*1卷积变换通道数据，同时包含可学习的缩放参数gamma
...
# CBS + 利用transpose2d恢复到原来的图像尺寸【padding 0后】
...
class Stem(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.cv1 = Conv(c1, c_, 3, 2) # CBS1，大小缩小为原来的一半
        self.cv2 = Conv(c_, c_, 1, 1) # CBS2，通道变换
        self.cv3 = Conv(c_, c_, 3, 2) # CBS3, 大小缩小为原来的一半
        self.pool = torch.nn.MaxPool2d(2, stride=2) # 大小缩小为原来的一半
        self.cv4 = Conv(2 * c_, c2, 1, 1) # 通道数映射

    def forward(self, x):
        x = self.cv1(x)
        return self.cv4(torch.cat((self.cv3(self.cv2(x)), self.pool(x)), dim=1))

# YOLOv3中的空间金字塔 图像大小缩小为原来的1/2
...
# 将不同kernel size大小的maxpool结果进行聚合
class SPP(nn.Module):
    # Spatial pyramid pooling layer used in YOLOv3-SPP
    def __init__(self, c1, c2, k=(5, 9, 13)):
        ...
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k]) # 形状大小不变的maxpool

    def forward(self, x):
        x = self.cv1(x) #
        return self.cv2(torch.cat([x] + [m(x) for m in self.m], 1))
    
#Bottleneck，如果c1==c2可以支持残差
...
class Ghost(nn.Module):
    ...
    def forward(self, x):
        return self.conv(x) + self.shortcut(x) # 不管s是否等于2，第一项的通道数肯定c2；对于第二项，如果s==2，那就是c2，否则为c1，无法相加。
